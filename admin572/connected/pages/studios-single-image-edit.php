<?php
$idImg = $_GET['id'];
$stmt = $db_client->prepare("SELECT * FROM studios_images WHERE id=? ORDER BY updated_at DESC");
$stmt->execute(array($idImg));
$studioImages = $stmt->fetchAll();

if (empty($studioImage))
    foreach ($studioImages as $studioImage) :
        $image = $studioImage->image;
        $image_alt = $studioImage->image_alt;
    endforeach;
...
if (!empty($_POST)) {
    ...
    if (!empty($image)) {
        $stmt = $db_client->prepare("UPDATE studios_images SET image=?, image_alt=?, updated_at= NOW() WHERE id=?");
        $stmt->execute(array($image, $image_alt, $idImg));
        $id_studio = $studioImage->id_studio;
        header("Location: studios-images-edit-" . $id_studio);
    }
}
?>

<h1 class="no-pt">Éditer l'image du studio</h1>
<hr><br>
<form role="form" method="post" action="#" enctype="multipart/form-data">
    <div class="carousel-blocs" style="flex-direction: column">
        <div class="bloc" style="flex-direction: column">
            ...
            <input id="image_alt" name="image_alt" class='form-control' type="text" value="<?= $image_alt ?>" placeholder="Ex : Voiture Jaune..." autofocus>
        </div>
    </div>
    </div><br>
    <hr>
