...
$stmt = $db_client->prepare("SELECT category.id categoryId, blocks1.title projectBlocks1Title FROM project blocks1 INNER JOIN project_categories category ON category.id = blocks1.id_project_category ORDER BY blocks1.created_at ASC"); // on prépare notre requête
$stmt->execute();
$projectsBlock1Images = $stmt->fetchAll();
